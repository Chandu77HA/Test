    ...
    for line in lines:
        ...
        if numbered_match:
            if not in_list or list_type != 'ol':
                ...
                list_type = 'ol'
            formatted_lines.append(
                f'<li>{numbered_match.group(2).strip()}</li>')
            ...
            if asterisk_items:
                ...
                formatted_lines.append(
                    f'<li>{asterisk_match.group(2).strip()}</li>')
        ...
        if request.FILES.get('pdf_file'):
            ...
            if user_input:
                try:
                    ...
                    #     response_synthesizer=response_synthesizer
                    # )
                    response__2 = doc_summary_index.as_query_engine(
                        text_qa_template=text_qa_template).query(query_2)
                    answer = formatted_answer(str(response__2))
